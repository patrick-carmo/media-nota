function nota(competencia) {
  ...
  for (let i = competencia; i <= (competencia === 1 ? 1 : 3); i++) {
    let questionario = parseFloat(prompt(`Digite a nota ${i} do questionário: `))
  ...
  if (notas < 17.5) {
    const recuperacao = parseFloat(prompt(`Digite a nota da recuperação da competência ${competencia}: `))
...
if (media < 60) {
  const substitutiva = parseFloat(prompt('Digite a nota da prova substitutiva: '))
